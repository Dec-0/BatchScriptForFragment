...
require Exporter;
@ISA = qw(Exporter);
@EXPORT = qw(PureNumSort NumStringSort AllSubSet);
...
{
	my @tRef = @_;
	die "[ Error ] NumStringSort requires 2 parameters.\n" unless($#tRef == 1);
	my @Num = @{$tRef[0]};
	my @String = @{$tRef[1]};
		...
		for(my $i = 0;$i <= $To;$i += $DulSpan)
		{
			...
			if($RightBegin <= $To)
			{
				...
				while($LeftBegin <= $LeftEnd || $RightBegin <= $RightEnd)
				{
					...
					{
						if($Num[$Index[$Id1][$LeftBegin]] > $Num[$Index[$Id1][$RightBegin]])
						{
							$Index[$Id2][$tmpId] = $Index[$Id1][$RightBegin];
							$RightBegin ++;
						}
	...
	my (@FinalNum,@FinalString) = ();
	for my $i (0 .. $#{$Index[$Id1]})
	{
		push @FinalNum, $Num[$Index[$Id1][$i]];
		push @FinalString, $String[$Index[$Id1][$i]];
	}

...
require Exporter;
@ISA = qw(Exporter);
@EXPORT = qw(ChrPosAndOther ChrPosAndOtherByNum);
...
sub ChrPosAndOtherByNum
{
	...
	for my $i (0 .. $#Chr)
	{
		$TransChr[$i] = $Chr[$i];
		$TransChr[$i] =~ s/^chr//i;
		$TransChr[$i] = 0 if($TransChr[$i] eq "M" || $TransChr[$i] eq "m");
		$TransChr[$i] = 23 if($TransChr[$i] eq "X" || $TransChr[$i] eq "x");
		$TransChr[$i] = 24 if($TransChr[$i] eq "Y" || $TransChr[$i] eq "y");
	}
		...
		for(my $i = 0;$i <= $To;$i += $DulSpan)
		{
			...
			if($RightBegin <= $To)
			{
				...
				while($LeftBegin <= $LeftEnd || $RightBegin <= $RightEnd)
				{
					...
					{
						if($TransChr[$Index[$Id1][$LeftBegin]] > $TransChr[$Index[$Id1][$RightBegin]] || ($TransChr[$Index[$Id1][$LeftBegin]] eq $TransChr[$Index[$Id1][$RightBegin]] && $From[$Index[$Id1][$LeftBegin]] > $From[$Index[$Id1][$RightBegin]]) || ($Chr[$Index[$Id1][$LeftBegin]] eq $Chr[$Index[$Id1][$RightBegin]] && $From[$Index[$Id1][$LeftBegin]] == $From[$Index[$Id1][$RightBegin]] && $To[$Index[$Id1][$LeftBegin]] > $To[$Index[$Id1][$RightBegin]]))
						{
							$Index[$Id2][$tmpId] = $Index[$Id1][$RightBegin];
							$RightBegin ++;
						}
